...
<div class="main_content">
	<%-- 공연번호 저장용 --%>
	<form action="/ticketing/mediapayment" method="post" id="ticketingForm">
							...
							<td>
								...
								<select id="showDate" name="showDate" class="form-select">
									...
									<c:forEach items="${ticketingDateList }" var="date">
										...
										<c:set value="30" var="mediaRestTicketChecked" />
										<c:forEach items="${mediaRestTicketList }" var="mediaRestTicket">
											<fmt:formatDate value="${mediaRestTicket.showDate }" pattern="yyyy-MM-dd" var="tempTicketDate"/>
											<c:if test="${seatChecked == 0 }">
												<c:choose>
													<c:when test="${tempDate == tempTicketDate && mediaRestTicket.ticketingRound == 1}">
														<c:set var="seatChecked" value="1" />
														<c:set var="mediaRestTicketChecked1" value="${mediaRestTicket.ticketingAmount }" />
													</c:when>
													<c:when test="${tempDate == tempTicketDate && mediaRestTicket.ticketingRound == 2}">
														<c:set var="seatChecked" value="2" />
														<c:set var="mediaRestTicketChecked2" value="${mediaRestTicket.ticketingAmount }" />
													</c:when>
												</c:choose>
											</c:if>
											<%-- 같은 날짜에서 다른 회차 둘다 예매내역이 있을 경우 --%>
											<c:if test="${seatChecked == 1 }">
												<c:if test="${mediaRestTicket.ticketingAmount == 2}">
										...
										<c:choose>
											...
											<c:when test="${seatChecked == 1 }">
												<option value="<fmt:formatDate pattern="yyyy-MM-dd" value="${date }" />" class="round1">
													=====<fmt:formatDate pattern="yyyy-MM-dd" value="${date }" />:${mediaRestTicketChecked1 }:=====
												</option>
											...
											<c:when test="${seatChecked == 2 }">
												...
												<option value="<fmt:formatDate pattern="yyyy-MM-dd" value="${date }" />" class="round2">
													=====<fmt:formatDate pattern="yyyy-MM-dd" value="${date }" />:${mediaRestTicketChecked2 }:=====
												</option>
											</c:when>
